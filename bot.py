...
from telegram.ext import (
    ...
    MessageHandler, filters, ContextTypes, CallbackQueryHandler, PollHandler
)
...
ristoranti = {
    'Ristorante A': {'link': 'https://goo.gl/maps/exampleA', 'rating': None, 'total_votes': 0},
    'Ristorante B': {'link': 'https://goo.gl/maps/exampleB', 'rating': None, 'total_votes': 0},
}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Benvenuto! Scegli un'opzione dal menu:", reply_markup=reply_markup)
...
async def choose_posizione(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not ristoranti:
        ...
        return
    keyboard = [[InlineKeyboardButton(r, callback_data=f"posizione:{r}")] for r in ristoranti]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Seleziona un ristorante per visualizzare la posizione:", reply_markup=reply_markup)
...
async def choose_sondaggio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not ristoranti:
        await update.message.reply_text("Errore: Nessun ristorante disponibile.")
        return
    keyboard = [[InlineKeyboardButton(r, callback_data=f"sondaggio:{r}")] for r in ristoranti]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Seleziona un ristorante per avviare il sondaggio:", reply_markup=reply_markup)
...
async def choose_elimina(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not ristoranti:
        await update.message.reply_text("Errore: Nessun ristorante disponibile.")
        return
    keyboard = [[InlineKeyboardButton(r, callback_data=f"elimina:{r}")] for r in ristoranti]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Seleziona un ristorante da eliminare:", reply_markup=reply_markup)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'lista':
        ...
        await query.edit_message_text(text="Per aggiungere un ristorante, usa il comando /aggiungi_ristorante.")
    elif data == 'help':
        help_text = (
            "Comandi disponibili:\n"
            "/start - Mostra il menu interattivo\n"
        ...
        if restaurant in ristoranti:
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text(text=f"Posizione per {restaurant}:", reply_markup=reply_markup)
        else:
        ...
        if restaurant in ristoranti:
            ...
            await query.edit_message_text(text=f"Sondaggio per {restaurant} creato con successo!")
        else:
            await query.edit_message_text(text="Errore: ristorante non trovato.")
        ...
        if restaurant in ristoranti:
            ...
            await query.edit_message_text(text=f"Ristorante '{restaurant}' eliminato con successo.")
        else:
            await query.edit_message_text(text="Errore: ristorante non trovato.")
    else:
...
async def aggiungi_ristorante_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "maps.google" in link or "goo.gl" in link or "google.com/maps" in link:
        ...
        await update.message.reply_text("Errore: Link non valido. Assicurati che sia un link di Google Maps.")
    return ConversationHandler.END

async def aggiungi_ristorante_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def poll_update_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if poll.id in poll_mapping:
        ...
        if total_votes > 0:
            rating_sum = sum((i + 1) * option.voter_count for i, option in enumerate(poll.options))
            average = rating_sum / total_votes
        else:
            average = 0
        ristoranti[restaurant]['rating'] = average
        ristoranti[restaurant]['total_votes'] = total_votes
        logger.info(f"Updated rating for {restaurant}: {average:.1f} from {total_votes} votes")

### SETUP DELL'APPLICAZIONE ###

def main():
    application = Application.builder().token("7360553682:AAHEVY2tjiv6bO4yGoJO7X5mrlCDNwAeZJ8").build()

    conv_handler = ConversationHandler(
        entry_points=[CommandHandler('aggiungi_ristorante', aggiungi_ristorante_start)],
        states={
            RIST_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, aggiungi_ristorante_nome)],
            RIST_LINK: [MessageHandler(filters.TEXT & ~filters.COMMAND, aggiungi_ristorante_link)]
        },
